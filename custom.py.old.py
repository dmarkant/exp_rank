...
def get_partner_data():
    ...
    # fix double-loaded participant data
    start_ind = 0
    for i in range(len(partner_data['data'])):
        if partner_data['data'][i]['trialdata'][0] == 'requesting partner data':
            start_ind = i

    for d in partner_data['data'][start_ind:]:
        if d['trialdata'][0] in ["study", "study_cond", "image_type", "activeitems", "yokeditems", "activeimages", "yokedimages", "testitems"]:
            study_data.append(d)
